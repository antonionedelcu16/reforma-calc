  ...
  const SECCIONES = [
    {
      ...
      badge: config.contactEmail ? "Activo" : "Sin configurar",
      badgeColor: config.contactEmail ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500",
      contenido: (
        <div className="text-sm text-slate-500">
          Los leads se envían a <strong className="text-slate-700">{config.contactEmail || "sin email configurado"}</strong>.
          Cámbialo en <a href="/admin/apariencia" className="text-orange-500 hover:underline">Apariencia → Datos de empresa</a>.
        </div>
      ),
    },
  ];
  ...
  return (
    <div className="p-8 max-w-3xl">
      <div className="mb-6">
        ...
        <p className="text-slate-500 mt-1">Conecta tu calculadora con tus herramientas de ventas</p>
      </div>
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${config.contactEmail ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"}`}>
          ...
          <div className="text-sm text-slate-500 bg-slate-50 rounded-xl p-3 border border-slate-100">
            Los leads llegan a <strong className="text-slate-700">{config.contactEmail || "—"}</strong> con todos los datos del cliente y el presupuesto consultado.
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${form.whatsappNumero ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"}`}>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${form.webhookUrl ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"}`}>
          ...
          <div className="bg-orange-50 border border-orange-200 rounded-xl p-3 mb-4 text-xs text-orange-700">
            <strong>Cómo obtener el webhook:</strong> En GHL → Configuración → Webhooks → Crear webhook → Copia la URL y pégala aquí. Los leads se crearán como contactos automáticamente.
          </div>
          <div className="mb-3">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${form.airtableToken && form.airtableBaseId ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"}`}>
          ...
          <div className="bg-purple-50 border border-purple-200 rounded-xl p-3 mb-4 text-xs text-purple-700">
            <strong>Cómo configurarlo:</strong> En Airtable → tu cuenta → Developer Hub → Personal access token → Crea uno con permisos <em>data.records:write</em>.
            El Base ID está en la URL de tu base: airtable.com/<strong>appXXXXXXX</strong>/...
          </div>
